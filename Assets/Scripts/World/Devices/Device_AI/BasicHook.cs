...
{
    public class BasicHook : Device, IAim
    {
            ...
            switch (m_fsm)
            {
                ...
                case Device_FSM_Hook.Ready_to_Shoot:
                    ...
                    if (target != null)
                    {
                        ...
                        if (parabolic_trajectory_prediction(muzzel_pos, BattleUtility.get_target_colllider_pos(target), move_speed_shooting, out var expt_dir))
                            ...
                            target_dir = get_target_collider_pos_with_aiming_y_offset() - muzzel_pos;

                        rotate_bone_to_dir(BONE_FOR_ROTATION, target_dir);

                        if (can_shoot(target_dir))
                        {
                            FSM_change_to(Device_FSM_Hook.Shooting);
                            interval = interval_max;
                            hook_v = move_speed_shooting * target_dir.normalized;
                        }
                    }
                ...
                case Device_FSM_Hook.Hooked:
                    ...
                    else
                    {
                        ...
                        if (WorldContext.instance.is_need_reset)
                            rope_current_length = (collider_pos_of_target - muzzel_pos - new Vector2(WorldContext.instance.reset_dis, 0)).magnitude;
                        else
                            rope_current_length = (collider_pos_of_target - muzzel_pos).magnitude;

                        if (Config.current.rope_min_length < rope_limit_length)
                        {
                            rope_limit_length = Mathf.Max(rope_limit_length - move_speed_hooked * Config.PHYSICS_TICK_DELTA_TIME, Config.current.rope_min_length);
                        }
        ...
        {
            return interval <= 0 && can_shoot_check_dir(target_dir);
        }

        private bool can_shoot_check_dir(Vector2 target_dir)
        {
        ...
        private Vector2 get_target_collider_pos_with_aiming_y_offset()
        {
            return BattleUtility.get_target_colllider_pos(target) + new Vector2(0, AIMING_Y_OFFSET);
        }
