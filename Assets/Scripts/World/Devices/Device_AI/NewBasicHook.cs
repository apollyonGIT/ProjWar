...
namespace World.Devices.Device_AI
{
    public class NewBasicHook : Device, IAttack, IRecycle, ILoad
    {
        ...
        public override void tick()
        {
            ...
            switch (m_fsm)
            {
                ...
                case Device_FSM_Hook.Hooked:
                    ...
                    if (hook_target == null || hook_target.hp <= 0 || !hook_target.is_interactive)
                    {
                        end_traction();
        ...
        {
            InputController.instance.RemoveLeftHold(Fire);
        ...
        private void Fire()
        {
            if (Current_Interval <= 0 && fsm == Device_FSM_Hook.Ready_to_Shoot)
            {
                Vector2 dir = bones_direction[BONE_FOR_ROTATION];
                FSM_change_to(Device_FSM_Hook.Shooting);
                Current_Interval = Attack_Interval;
                hook_v = move_speed_shooting * dir.normalized;
            }
        }
