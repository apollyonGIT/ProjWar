...
namespace World.Devices.NewDevice
{
    public class BasicShield : Device, IAttack, IShield
    {
        ...
        public override void tick()
        {
            ...
            switch (fsm)
            {
                case Device_FSM_Shield.idle:
                   ...
                   /* Vector2 pos_to_pointer = InputController.instance.GetWorlMousePosition() - new Vector3(position.x, position.y, 10);
        ...
        {
            target = BattleUtility.select_target_in_circle_min_angle(position, bones_direction[BONE_FOR_ROTATION], desc.basic_range.Item2, faction, (ITarget t) =>
            {
        ...
        public override void StartControl()
        {
            if (can_blaze)
            {
                InputController.instance.left_hold_event += Melee_Attack;
                InputController.instance.left_click_event += Melee_Attack;
            }
            else
                InputController.instance.left_click_event += Melee_Attack;
        ...
        {
            if (InputController.instance.holding_right && can_manual_attack())
